...
/* =========================================================
   天気取得（安定版）
========================================================= */
const weatherCache: Record<string, { data: WeatherResponse; ts: number }> = {};
...
export const fetchWeatherData = async (latitude: string, longitude: string): Promise<WeatherResponse> => {
  ...
  const response = await fetch(
    `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&hourly=temperature_2m,wind_speed_10m,precipitation_probability,weather_code&daily=temperature_2m_max,temperature_2m_min,precipitation_probability_max,weathercode&forecast_days=7&timezone=Asia%2FTokyo`
  );
